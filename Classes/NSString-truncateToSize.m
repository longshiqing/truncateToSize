...
- (CGSize) hj_sizeWithFont:(UIFont *)fontToUse
{
...
- (NSString *)truncateToSize: (CGSize)size withFont: (UIFont *)font lineBreakMode: (UILineBreakMode)lineBreakMode withStartingAnchor: (NSString *)startingAnchor withEndingAnchor: (NSString *)endingAnchor {
    if( !( lineBreakMode & ( UILineBreakModeHeadTruncation | UILineBreakModeMiddleTruncation | UILineBreakModeTailTruncation ) ) )
        return self;
    if( [self hj_sizeWithFont: font].width <= size.width )
        return self;
    ...
    if( [[self substringWithRange: NSMakeRange( start, targetLength )] hj_sizeWithFont: font].width < [ellipsis hj_sizeWithFont: font].width )
    {
    ...
    switch( lineBreakMode ) {
        case UILineBreakModeHeadTruncation:
            ...
            if( startingAnchor )
                start++;
            while( targetLength > [ellipsis length] + 1 && [truncatedString hj_sizeWithFont: font].width > size.width) {
        ...
        case UILineBreakModeMiddleTruncation:
        {
            ...
            while( leftEnd > start + 1 && rightStart < end + 1 && [truncatedString hj_sizeWithFont: font].width > size.width) {
                CGFloat leftLength = [left hj_sizeWithFont: font].width;
                CGFloat rightLength = [right hj_sizeWithFont: font].width;
